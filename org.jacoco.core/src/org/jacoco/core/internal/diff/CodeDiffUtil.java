...
public class CodeDiffUtil {
    ...
    public static boolean isJava(String path) {
        return !StringUtils.isNullOrEmpty(path) && path.toLowerCase().endsWith(".java");
    }

    public static boolean isKotlin(String path) {
        return !StringUtils.isNullOrEmpty(path) && path.toLowerCase().endsWith(".kt");
    }
...
//	}
     ...
     * @return Boolean
     */
    public static Boolean checkMethodIn(String className, String methodName, String desc) {
        // 参数校验
        if (CollectionUtils.isNullOrEmpty(CoverageBuilder.classInfos)
                || StringUtils.isNullOrEmpty(methodName)
                || StringUtils.isNullOrEmpty(className)) {
            return Boolean.FALSE;
        }
        ClassInfo classInfo = Utils.getClassInfoByClassName(className);
        if (null == classInfo) {
            return Boolean.FALSE;
        }
        // 如果是新增类，不用匹配方法，直接运行
        if (StringUtils.equalsIgnoreCase(org.jacoco.core.internal.diff.Type.ADD, classInfo.getType())) {
            return Boolean.TRUE;
        }
        Map<String, MethodInfo> methodInfos = classInfo.getMethodInfos();
        if (CollectionUtils.isNullOrEmpty(methodInfos)) {
            return Boolean.FALSE;
        }
        // 匹配了方法，参数也需要校验
        try {
            return methodInfos.values().stream().anyMatch(m -> {
                if (methodName.equals(m.getMethodName())) {
                    return checkParamsIn(m.getParametersList(), desc);
                } else {
                    return Boolean.FALSE;
                }
            });
    ...
    public static Boolean checkParamsIn(List<String> params, String desc) {
        ...
        // 说明是无参数的方法，匹配成功
        if (CollectionUtils.isNullOrEmpty(params) && argumentTypes.length == 0) {
            return Boolean.TRUE;
        }
